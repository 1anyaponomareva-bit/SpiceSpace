...
COACH_STYLE_INSTRUCTION = """АНАЛИЗ СОСТОЯНИЯ ПОЛЬЗОВАТЕЛЯ (выполнять перед каждым ответом, молча):

Определи текущий режим по последним 2-3 сообщениям:

РЕЖИМ "ЭНЕРГИЯ" — признаки:
- Короткие чёткие фразы
- "давай", "хочу", "готова", "окей", "поехали"
- Конкретные вопросы или планы
- Восклицания, уверенный тон

В этом режиме: конкретика, движение вперёд, можно слегка подтолкнуть,
не сюсюкай, ставь чёткие задачи.

РЕЖИМ "ПОДДЕРЖКА" — признаки:
- "не знаю", "устала", "не могу", "всё плохо", "запуталась"
- Длинные размытые сообщения
- Извинения за себя или за невыполненное
- Молчала несколько дней и вернулась
- Тревога, усталость, потерянность

В этом режиме: НЕ давай задачи, НЕ толкай вперёд.
Просто будь рядом. Спроси как она.
Максимум одно мягкое предложение в конце если сама готова двигаться.

ВАЖНО:
- Переключайся между режимами каждый раз заново по текущим сообщениям
- Никогда не фиксируй режим за человеком навсегда
- Не говори пользователю в каком режиме ты сейчас
- Переход естественный, как у подруги которая просто чувствует

ТИПЫ ЦЕЛЕЙ:
Если цель СТРАТЕГИЧЕСКАЯ (цифры, метрики, конкретный результат) —
работай через конкретные задачи и измеримые шаги.

Если цель МЕНТАЛЬНАЯ (меньше тревоги, найти себя, спокойствие, баланс) —
НЕ требуй цифр и метрик.
Утренний вопрос: "Как ты сегодня?" вместо "Что планируешь?"
Вечерний вопрос: "Что хорошего было сегодня?" вместо "Выполнила ли задачу?"
Прогресс через ощущения, не через цифры.

Ты ведёшь диалог на русском как живой человек: просто, по делу, без шаблонов и «мотивационных» речей.
...
_WEEKTEST_ADMINS = {
    x.strip()
    for x in (
        os.getenv("WEEKTEST_TELEGRAM_IDS", "8412438788") or ""
    ).split(",")
    if x.strip()
}


async def cmd_weektest(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Dev helper: simulate week end (day 7) or week start (day 8) for your cycle."""
    if not update.effective_chat or not update.message:
        return
    cid = update.effective_chat.id
    if str(cid) not in _WEEKTEST_ADMINS:
        await _bot_reply(
            update.message,
            "Команда только для тестового аккаунта.",
        )
        return
    prof = user_profiles.get(str(cid)) or db_store.get_profile(cid)
    if not isinstance(prof, dict):
        await _bot_reply(update.message, "Сначала /start — нужен профиль.")
        return
    ...
    arg = (context.args[0] if context.args else "status").strip().lower()
    if arg == "status":
        prof = _ensure_cycle_start_date(cid, prof, today)
        dss = _days_since_cycle_start(prof, today)
        journey = _program_journey_days(prof)
        prev_key = (
    ...
    if arg == "end":
        ...
        await _bot_reply(
            update.message,
            f"Ок. cycle_start_date={start} (сегодня день 7).\n"
            "Дождись вечернего времени или напиши боту вечером — начнётся диалог итогов.",
        )
        return
    if arg == "start":
        ...
        for k in list(prof.keys()):
            if str(k).startswith("weekly_sent_day_") or str(k).startswith(
                "new_week_sent_day_"
            ):
                prof.pop(k, None)
        cf = prof.get("cycle_flags")
        if isinstance(cf, dict):
            prof["cycle_flags"] = {
                k: v
                for k, v in cf.items()
                if not str(k).startswith(("weekly_sent_day_", "new_week_sent_day_"))
            }
        db_store.update_profile(cid, {"cycle_flags": prof.get("cycle_flags") or {}})
        user_profiles[str(cid)] = db_store.get_profile(cid) or prof
        await _bot_reply(
            update.message,
            f"Ок. cycle_start_date={start} (сегодня день 8).\n"
            "Утром придёт запрос новой недельной цели; после неё — задачи на день.",
        )
        return
    if arg == "recap":
        prof = _ensure_cycle_start_date(cid, prof, today)
    ...
    if arg == "reset":
        ...
        await _bot_reply(
            update.message,
            "Сброс недельных флагов. cycle_start = сегодня (день 1).",
        )
        return
    await _bot_reply(
        update.message,
...
def _recent_dialog_snippet(chat_id: int, *, ru: bool, max_turns: int = 10) -> str:
    user_l = "Она" if ru else "User"
    space_l = "Спейс" if ru else "Space"
    lines: list[str] = []
    for turn in (histories.get(chat_id, []) or [])[-max_turns:]:
        role = turn.get("role")
        content = turn.get("content") or (turn.get("parts") or [""])[0]
        text = str(content or "").strip()
        if not text:
            continue
        who = user_l if role == "user" else space_l
        lines.append(f"{who}: {text[:300]}")
    return "\n".join(lines) if lines else ("пока мало контекста" if ru else "little context yet")


def _task_day_followup_fallback(profile: dict, today_task: str) -> str:
    lang = str(profile.get("language_code") or "en")
    ru = lang.lower().startswith("ru")
    task = (today_task or "").strip() or ("задачу" if ru else "the task")
    if ru:
        return (
            f"Про «{task}» — с чего начнёшь? "
            f"Расскажи вслух, помогу разложить по шагам 💚"
        )
    return (
        f"About «{task}» — where will you start? "
        f"Talk me through it, I'll help you break it down 💚"
    )
...
async def _task_day_followup_text(
    ...
    *,
    ...
    lang = str(profile.get("language_code") or "en")
    ru = lang.lower().startswith("ru")
    weekly_goal = str(profile.get("weekly_goal") or "").strip() or (
        "не указана" if ru else "not specified"
    )
    ...
    def call() -> str:
        for mid in model_names:
            try:
                ...
                log.warning("task_day_followup %s: %s", mid, e)
...
def _days_since_cycle_start(profile: dict, today: date) -> int | None:
    cycle_start_raw = str(profile.get("cycle_start_date") or "").strip()
    if not cycle_start_raw:
        return None
    try:
...
def _ensure_cycle_start_date(
    cid: int, profile: dict, today: date
) -> dict:
    """Backfill cycle_start_date for profiles created before the field existed."""
    if str(profile.get("cycle_start_date") or "").strip():
        return profile
    journey = _program_journey_days(profile)
    if journey is None:
        summaries = db_store.list_daily_summaries(str(cid))
        dates = [
            str(s.get("summary_date") or s.get("date") or "")[:10]
            for s in summaries
    ...
    async def daily_check_job() -> None:
        try:
            ...
            for cid in list(subscribers):
                ...
                profile = _ensure_cycle_start_date(
                ...
                if in_evening:
                    ...
                    if is_week_end and days_since_start is not None:
                        weekly_sent_key = f"weekly_sent_day_{days_since_start}"
